...
CommsAT <- data.frame(matrix(0, nrow = nSpecies,ncol = nComms))
...
CommsS <- round(SeedlPerTree*exp(CNDD*(nPlotObs/nCommObs)*CommsAT))
...
x <- (nPlotObs/nCommObs)*c(t(CommsAT[i,seq(from = 1, to = n.reps*n, by = n.reps)]))
...
model {
  ...
  // model structure  
  for (i in 1:nobs){
    xobs[i] ~ normal(xmeans[i], sigmax);
  }
  for (i in 1:n){
    y[i] ~ normal(exp(alpha) + exp(beta*x[i]), sigmay);
  }
}
...
stan_d <- c("y", "xobs", "nobs", "n", "indx","xmeans")
chains <- 3
iter <- 1000
thin <- 1
